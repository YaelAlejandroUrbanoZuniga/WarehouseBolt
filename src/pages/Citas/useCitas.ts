...
import { citasAtom, transicionesAtom, transportistasAtom, proveedoresAtom, usuarioActivoAtom } from '@/lib/store';
...
export function useCitas() {
  ...
  const crearCita = useCallback((input: CitaInput): { ok: true } | { ok: false; motivo: string } => {
    ...
    return { ok: true };
  }, [citas, transiciones, transportistas, proveedores, usuarioActivo, setCitas, setTransiciones]);
