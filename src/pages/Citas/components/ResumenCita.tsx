...
import { docksAtom, rolActivoAtom, transportistasAtom, proveedoresAtom } from '@/lib/store';
...
export function ResumenCita({ cita, transiciones, onEditarCita, onCancelarCita, onBorrarCita, onClose }: Props) {
  ...
  const opcionesTransporte = useMemo(() => transportistas.map(t => t.empresa), [transportistas]);
  const opcionesProveedor = useMemo(
    () => proveedores.filter(p => p.activo).map(p => p.nombre),
    [proveedores],
  );
  ...
  const [proveedorLabel, setProveedorLabel] = useState(cita.proveedorNombre ?? '');
  ...
  function iniciarEdicion() {
    ...
    setProveedorLabel(cita.proveedorNombre ?? '');
  ...
  function guardar() {
    onEditarCita(cita.id, {
      ...
      proveedorNombre: proveedorLabel.trim(),
      origen: origen.trim(),
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 24px', fontSize: 13 }}>
          <InfoRow label="Proveedor" value={cita.proveedorNombre || '—'} />
          <InfoRow label="Línea de transporte" value={cita.empresa || '—'} />
          <InfoRow label="Origen" value={cita.origen} />
          <InfoRow label="Destino" value={cita.destino} />
          <InfoRow label="PO" value={cita.poNumero} />
        </div>
        {esRetraso && (
